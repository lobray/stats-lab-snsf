...
calc_pseudo_r <- function(log_regression_object) {
  round((log_regression_object$null.deviance - log_regression_object$deviance) /
          (log_regression_object$df.null + log_regression_object$null.deviance - log_regression_object$deviance),4)
...
```{r}
## NEED TO UPDATE TO USE STANDARDIZED VARIABLES. BUT, WHAT DOES A STANDARDIZED BINARY VARIABLE EVEN MEAN? CHECK IF USEMODEL=F
# IS SUFFICIENT
...
QUESTION: How can we assess the interactions? Since we can only permute the explanatory variablse that aren't interactions? Or that doesn't really matter since the end goal is the item (Gender) and not the specific interaction of Gender+Division?
...
calc_variable_importance <- function(predictors = predictors, data = board_data, regression_object) {
  variable_imp_matrix <- cbind(precitors, rep(0, length(predictors)))
  base_pseudo_r <- calc_pseudo_r(regression_object)
  
  for (i in predictors) {
    permuted_predictor <- permute_values(board_data[,predictor[i]])
    permuted_board_data <- board_data
    permuted_board_data[,predictor[i]] <- permuted_predictor
