...
table(board_data$OverallGrade, board_data$Ranking)
table(board_data$OverallGrade, board_data$IsApproved)

board_data$Ranking <- ifelse(board_data$Ranking < 4, 3, board_data$Ranking)
board_data$OverallGrade <- ifelse(board_data$OverallGrade < 4, 3, board_data$OverallGrade)
...
board_log_regression <- glm(board_data$IsApproved ~ Gender + Division + Age + IsContinuation + InstType + log(AmountRequested) + 
                              Ranking + OverallGrade + Gender:Division + PercentFemale*Gender, family="binomial", data = board_data)
...
table(board_data$IsApproved, board_data$OverallGrade)
```
