...
We have three data sets: Applications, External Reviewers and Internal Referees. They contain respectively information about the SNSF project funding applications, the evaluation of the applications by external peer reviewers and the evaluation of the proposals by external the internal referee and co-referee (when available).
...
external_regression_data<-prepare_data_external_log_regression(external=final.external)
...
internal_regression_data<-prepare_data_internal_log_regression(internal=final.internal)
...
kappa.internal <- prepare_data_internal_log_regression(final.apps,final.internal)
kappa.external <- prepare_data_external_log_regression(final.apps, final.external)
candidate.rating <- merge(x=kappa.external[,c("ProjectID", "ApplicantTrack")], y = kappa.internal[,c("ProjectID", "ApplicantTrack")],
                      by="ProjectID")
...
kappa.matrix <- matrix(0, nrow=6, ncol=6)
colnames(kappa.matrix) <- 1:6
rownames(kappa.matrix) <- 1:6

# count number of times things agree/don't agree
for (i in 1:nrow(candidate.rating)) {
  kappa.matrix[candidate.rating[i,][[2]],candidate.rating[i,][[3]]] <- kappa.matrix[candidate.rating[i,][[2]],candidate.rating[i,][[3]]] + 1
}
?cohen.kappa
dep <- matrix(c(10, 4, 1, 6, 16, 2, 0, 3, 8), nrow = 3, byrow=T)
q.weights <- matrix(c(1, .75, 0, .75, 1, .75, 0, 0.75, 1),ncol=6, byrow=T)
dep.ck <- cohen.kappa(dep, w=q.weights) # quadratic weights
applicant.kappa <- cohen.kappa(kappa.matrix) # quadratic weights
