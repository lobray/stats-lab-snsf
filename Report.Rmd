...
Model.Overall<-clm(OverallGrade ~ ApplicantTrack + ProposalCombined + PercentFemale + 
    Gender + IsContinuation + PercentFemale:Gender, data=external_regression_data)
Over.nogender<- clm(OverallGrade ~ ApplicantTrack + ProposalCombined + PercentFemale + 
    IsContinuation, data=external_regression_data)
                            
p.value<-anova(Model.Overall,Over.nogender)[2,6]
```

* **Overall Grade**: To see which variable is the most important in determining the OverallGrade, we fitted a model, after variable selection, with OverallGrade as a Response, and predictors: ApplicantTrack, ProposalCombined, PercentFemale, Gender, IsContinuation and interaction between Gender and PercentFemale. Comparing this model with the one without gender suggest that gender is not significant to the model, p.value of `r p.value`. 
```{r}
...
# Combine grades 1,2,3 into a single category 
...
PsR2.small<-(1-exp((Model.log.int$dev-Model.log.int$null)/1623))/(1-exp(-Model.log.int$null/1623))
```
...
As said before, our focus is the effect of gender in each step of the evaluation process. We checked the difference in the probability of being accepted between male and female: women have probability `r prob[2,1]` of the project being approved, while men have `r prob[1,1]`. Surprisingly, the probability of being funded seems to be higher for female applicants, even if the difference is too small to be significant. We can clearly see that from the Gender effect plot, where the confidence intervals are overlapping and the line is almost horizontal. From this initial analysis we can say that it doesn't seem that referees are biased against women. 
...
```{r Logistic-internal-relative-importance, out.width='50%',fig.align="center"}
# Relative variable importance 
  load("Int.logistic_variable_importance.Rda")
  kable(PseudoRShuffle,"latex", booktabs = T) %>%
    kable_styling(position = "center")
...
To see which variables influence the different grades in the second step of the evaluation process and check if the gender of the main applicant has an influence on it, we ran an ordinal regression with the function clm of the package ordinal in R. We did this for both, the applicant grade (ApplicantTrack) and grade given to the project (ProjectAssessment). In order to avoid a multiple testing problem, we applied Bonferroni correction: since we performed 3 tests on the same dataset, we multiply the p-value obtained from the Likelihood Ratio Test to the number of tests performed.

```{r clm after variable selection}
internal_regression_data<-prepare_data_internal_log_regression(final.apps,final.internal)
internal_regression_data$logAmount <- log(internal_regression_data$AmountRequested)

# Fit the full model
fit <- clm(ProjectAssessment ~ Gender*Division+Gender*PercentFemale +Division*PercentFemale
           +Age+IsContinuation+ PreviousRequest+InstType+Semester+logAmount, data=internal_regression_data)
...
pvalue <- anova(Model2.project, Model.project)[2,6]*3
```
...
gender.prob<-Effect("Gender", mod=Model.project)
gender.prob<-gender.prob$prob
gender.prob<-data.frame(Male=gender.prob[1,],Female=gender.prob[2,])
rownames(gender.prob)<-c("poor","average","good", "very good",
                        "excellent","outstanding")
...
We kept gender as a predictor in our model to be able to show the effect it has on the grades. For this purpose, we estimated the cumulative probability of falling in the different grades for each gender. The result is presented in the next table. Overall the average difference is really small: `r round(mean(abs(gender.prob[,3])),5)`. We see here as well, that there is no evidence of gender influencing the probability of achiving a certain grade. From the plot below we can see the cumulative probability curves for male and female: they follow the same trend and the difference between the two is very small, even if it seems to increase with grades.
...
ggplot(gender.prob) +
          ...
          theme(legend.title=element_blank(),legend.position = "bottom")+
          xlab("")+ylab("Cumulative Probability")+
          ggtitle("Cumulative Probability for ProjectAssessment")
```

```{r, include=F}
load("Internal_Project_variable_Importance.Rda")
```
...
mycol=colorRampPalette(c("red","green"))(n)
       ...
       barplot(Shuffle.Result.Prop[,3], names.arg = Shuffle.Result.Prop[,1],
               horiz = TRUE,col = mycol,cex.names = 0.7,
               main="Variable Importance on Project Assessment",
               xlab = "Percent of variation on the AIC",
               xlim=c(0,0.7),
               cex.axis = 0.6,
               cex.main=0.8)
...
pvalue <- anova(Model2.track, Model.track)[2,6]*3
...
```{r, include=F}
kable(coeff<-round(summary(Model.track)$coefficients,4),"latex", booktabs = T)%>%
    kable_styling(position = "center")
```
...
gender.prob.track<-Effect("Gender", mod=Model.track)
gender.prob.track<-gender.prob.track$prob
gender.prob.track<-data.frame(Male=gender.prob.track[1,],Female=gender.prob.track[2,])
rownames(gender.prob.track)<-c("poor","average","good", "very good",
                        "excellent","outstanding")

gender.prob.track<-cbind(round(gender.prob.track,3),Difference=round((gender.prob.track[,1]-gender.prob.track[,2]),3))
```

    The average difference of the cumulative probability is here `r round(mean(abs(gender.prob.App[,3])),5)`, very close to zero. From the plot below it is not possible to see a difference between male and female, the lines overlap perfectly. This seems to suggest that the small p-value from the likelihood ratio test is not really reliable to establish whether there is gender bias.

```{r out.width='50%',fig.align="center"}
kable(gender.prob.track,"latex", booktabs = T) %>%
    kable_styling(position = "center")
...
```
```{r, include=F}
load("Internal_Track_variable_importance.Rda")
```
...
```{r}
kable(coeff<-round(summary(Model.Rank)$coefficients,4),"latex", booktabs = T)%>%
    kable_styling(position = "center")
```

```{r, include=F, message=F, warning=F}

gender.prob.Rank<-Effect("Gender", mod=Model.Rank)
gender.prob.Rank<-gender.prob.Rank$prob
gender.prob.Rank<-data.frame(Male=gender.prob.Rank[1,],Female=gender.prob.Rank[2,])
rownames(gender.prob.Rank)<-c("poor","average","good", "very good",
                        "excellent","outstanding")

gender.prob.Rank<-cbind(round(gender.prob.Rank,5),Difference=round((gender.prob.Rank[,1]-gender.prob.Rank[,2]),5))
       
```

    The predicted probabilities of achieving certain grade for male and female is shown in the next table. The average difference of the cumulative probability is here as well very close to zero (`r round(mean(abs(gender.prob.Rank[,3])),5)`). Notice that the difference is increasing with the grades and so the greatest difference on average is for grade "Outstanding" (`r gender.prob.Rank[6,3]`). From the plot below, we see that the trend is again the same for both genders and that the difference is not relevant.
    
```{r out.width='50%',fig.align="center"}
kable(gender.prob.Rank,"latex", booktabs = T) %>%
    kable_styling(position = "center")

ggplot(gender.prob.Rank) +
          geom_line(aes(1:6, Male, colour = "Male")) +
          geom_line(aes(1:6, Female, colour = "Female"))+
          theme(legend.title=element_blank(),legend.position = "bottom")+
          xlab("")+ylab("Cumulative Probability")+
          ggtitle("Cumulative Probability for the Ranking")
```  

```{r, include=F}
load("Internal_Ranking_variable_importance.Rda")
```
...
```{r out.width='50%', fig.align="center"}
kable(Shuffle.Result.R,"latex", booktabs = T) %>%
    kable_styling(position = "center")

n<-dim(Shuffle.Result.R)[1] 
mycol=colorRampPalette(c("red","blue"))(n)
       par(las=2)
       barplot(Shuffle.Result.R[,3], names.arg = Shuffle.Result.R[,1],
               horiz = TRUE,col = mycol,cex.names = 0.7,
...
The external step model is not a good explanation of the variation in the approval of applications. We nevertheless look into the influence of gender at this stage, but couldn't find evidence of its effect on the final decision.

When looking at the different grades in this step, we found that gender has a small influence in the Application track assessment, but as this variable is far less important than the project assessment in the determination of the Overall Grade, the influence vanishes out.

## Internal Step

The internal model is a quite good explanation of the variation in the approval of applications (around 70% of the variability is explained). When looking at each criteria of evaluation, we found that gender seems to have a small influence in the grade given, however when we checked the variable importance it always turned out to be one of the least important variable in the model. 


Considering all the analysis that we have done so far, we can say that there is no evidence of gender bias in the funding decision at the Swiss National Science Foundation. Gender is never an important variable in all the regressions we performed and the difference in predicted probability between male and female applicants is not relevant.
...
```{r}
  ...
  Model.int.step<-clm(Ranking ~ Gender + Division + PercentFemale + ProjectAssessment + 
...
When we permute the Project Assessment grades, the AIC increases by `r round(Shuffle.Result[8,3],2)`%. The second most important variable is the Applicant Track, but shuffling its values the AIC increases by "only" `r round(Shuffle.Result.R[7,3],2)`%.
...
```
```{r plot-effects-int, out.width='50%',fig.show="hold",fig.ncol=2, fig.cap="Effects of the external logistic regression", eval=F}
...
```
```{r plot-effects, out.width='50%',fig.show="hold",fig.ncol=2, fig.cap="Effects of the external logistic regression", eval=F}
...
plot(Effect("Age",Model.log.int),
       confint=list(style="band",alpha=0.3,col="grey"),
       lines=list(col=1))
plot(Effect("logAmount",Model.log.int),
       confint=list(style="band",alpha=0.3,col="grey"),
       lines=list(col=1))
