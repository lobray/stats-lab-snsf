...
# Fit the full model with all variables and interactions
  Model <- glm(IsApproved ~. +Gender:Division+Gender:PercentFemale+Gender:ApplicantTrack+InstType:Division,
...
log.int.Model <- glm(IsApproved ~ Gender + Age + Semester + IsContinuation + PercentFemale + 
    ApplicantTrack + ProjectAssessment + logAmount, data=data, family="binomial")

PsR2.small<-(1-exp((log.int.Model$dev-log.int.Model$null)/1623))/(1-exp(-log.int.Model$null/1623))
```
...
```{r effect plots}
# Obtain all the effects        
  eff.fit <- allEffects(log.int.Model)
  
# Obtain Gender effect as a separate data frame 
  eff<-Effect("Gender",log.int.Model)
  eff<-eff$fit
  prob<- round((exp(eff)/(1+exp(eff))),3)
```
...
```{r fig.align="center", fig.height=3}
 plot(Effect("Gender",log.int.Model),
       confint=list(style="band",alpha=0.3,col="grey"),
       lines=list(col=1))
```
...
Model <- clm(ProjectAssessment ~ Gender + Division + PercentFemale + Age + 
               IsContinuation + InstType + logAmount, data=internal_regression_data)

Model2 <- clm(ProjectAssessment ~ Division + PercentFemale + Age + 
                IsContinuation + InstType + logAmount, data=internal_regression_data)

# Compare the models using Bonferroni correction
pvalue <- anova(Model2, Model)[2,6]*3
```
...
gender.prob<-Effect("Gender", mod=Model)
gender.prob<-gender.prob$prob
gender.prob<-data.frame(Male=gender.prob[1,],Female=gender.prob[2,])
rownames(gender.prob)<-c("poor","average","good", "very good",
                        "excellent","outstanding")
...
Model2.App <- clm(ApplicantTrack ~ Division + PercentFemale + IsContinuation + 
               InstType + Semester + logAmount + Division:PercentFemale, data=internal_regression_data)

pvalue <- anova(Model2.App, Model.App)[2,6]*3
...
Model.R <- clm(Ranking ~  Gender + PercentFemale + Division + IsContinuation + 
               PreviousRequest + InstType + logAmount, data=internal_regression_data)

Model2.R <- clm(Ranking ~  PercentFemale + Division + IsContinuation + 
                PreviousRequest + InstType + logAmount, data=internal_regression_data)

pvalue <- anova(Model2.R, Model.R)[2,6]*3
...
```{r}
kable(coeff<-round(summary(Model.R)$coefficients,4),"latex", booktabs = T)%>%
    kable_styling(position = "center")
```

```{r, include=F, message=F, warning=F}

gender.prob.R<-Effect("Gender", mod=Model.R)
gender.prob.R<-gender.prob.R$prob
gender.prob.R<-data.frame(Male=gender.prob.R[1,],Female=gender.prob.R[2,])
rownames(gender.prob.R)<-c("poor","average","good", "very good",
                        "excellent","outstanding")

gender.prob.R<-cbind(round(gender.prob.R,5),Difference=round((gender.prob.R[,1]-gender.prob.R[,2]),5))
       
```

    The predicted probabilities of achieving certain grade for male and female is shown in the next table. The average difference of the cumulative probability is here as well very close to zero (`r round(mean(abs(gender.prob.R[,3])),5)`). Notice that the difference is increasing with the grades and so the greatest difference on average is for grade "Outstanding" (`r gender.prob.R[6,3]`). From the plot below, we see that the trend is again the same for both genders and that the difference is not relevant.
    
```{r out.width='50%',fig.align="center"}
kable(gender.prob.R,"latex", booktabs = T) %>%
    kable_styling(position = "center")

ggplot(gender.prob.R) +
          geom_line(aes(1:6, Male, colour = "Male")) +
          geom_line(aes(1:6, Female, colour = "Female"))+
          theme(legend.title=element_blank(),legend.position = "bottom")+
          xlab("")+ylab("Cumulative Probability")+
