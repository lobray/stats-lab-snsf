...
# To see variable importance based on pseudo r^2
  refR2<-calc_pseudo_r(log.ext.Model,nrow(data))
  PseudoRShuffle <- NULL
  ...
  for (feature in predictorNames) {
    ...
    for (iter in 1:shuffletimes) {
      shuffledData[,feature] <- sample(shuffledData[,feature],
                                       length(shuffledData[,feature]))
      Model.tmp <- update(log.ext.Model,.~.,data=shuffledData)
